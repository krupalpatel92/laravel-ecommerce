...
class UpdateProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $rules = [
            ...
            'type' => [
                ...
                function ($attribute, $value, $fail) use ($product) {
                    if ($product instanceof Product && $product->type !== $value) {
                        if ($product->variations()->exists()) {
                            $fail('Cannot change product type when variations exist');
                        }
                    }
                },
            ],
        ...
        if ($this->input('type') === 'single' || ($product instanceof Product && $product->type === 'single')) {
            $rules['price'] = ['sometimes', 'required', 'numeric', 'min:0'];
            $rules['stock_quantity'] = ['sometimes', 'required', 'integer', 'min:0'];
        }

        // For variable products, variations can be updated
        if ($this->input('type') === 'variable' || ($product instanceof Product && $product->type === 'variable')) {
            $rules['variations'] = ['sometimes', 'array', 'min:1'];
            $rules['variations.*.id'] = ['sometimes', 'exists:product_variations,id'];
